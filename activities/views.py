...
class ActivityViewSet(ViewSet):
    ...
    def get(self, request, telegram_id: int, *args, **kwargs):
        ...
        activity = Activity.objects.exclude(user=user_data[1]).filter(date_start__gte=datetime.date.today()).annotate(user_count=Count('user')).filter(users_max_count__gte=F('user_count')).values('id', 'name', 'town', 'date_start', 'user_count', 'users_max_count')
